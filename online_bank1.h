...
//
// Created by lwinh on 1/8/2024.
//

#ifndef INC_2024AVL_BANK_AVL_H
#define INC_2024AVL_BANK_AVL_H
...
void loadingAllDataFromFile(){
    ...
    FILE *fptr = fopen("C:/Users/lwinh/CLionProjects/2024AVL/ncc_db1.txt","r");
    ...
    if( fptr == NULL){
        ...
        for(int ncc=0; ncc<SIZE ; ncc++){
            //id name nrc email password pOrb loan_status monthly_income
            // loan_amount loan_rate accountStatus account_Level phNumber current_amount address TransRC
            fscanf(fptr ,"%u%s%s%s%s%s%s%u%u%f%s%d%u%u%s%d",&db[ncc].id,&db[ncc].name,&db[ncc].email,&db[ncc].nrc,
            ...
            for(int gcc=0; gcc<space_array[0]-15 ; gcc++){
...
void printingAllData(){
    ...
    for(int ncc=0; ncc<users ; ncc++){
        ...
        printf("%u-%s-%s-%s-%s-%s-%s-%u-%u-%f-%s-%d-%u-%u-%s-%d",db[ncc].id,db[ncc].name,db[ncc].email,db[ncc].nrc,db[ncc].password,
...
void recording_alldata_toFile(){
    FILE *fptr = fopen("C:/Users/lwinh/CLionProjects/2024AVL/ncc_db1.txt","w");
    if(fptr==NULL){
